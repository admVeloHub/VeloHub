 ...
 * 
 * Processa aba "Base Bacen 2025" do arquivo XLSX e insere na collection reclamacoes_bacen
 * Todos os registros são marcados como Finalizado.Resolvido = true
 * 
 * Uso:
 *   node backend/scripts/migrate-bacen-2025-xlsx.js [--dry-run]
 * 
...
function converterRegistro(row) {
  ...
  let nome = nomeCompleto ? String(nomeCompleto).trim() : '';
  if (!nome) {
  ...
  return {
    ...
    produto: buscarCampo(row, ['Produto']) || '', // Pode não existir em 2025
    anexos: [],
 ...
 */
async function main() {
  console.log('🚀 Iniciando migração Base Bacen 2025 (XLSX) → MongoDB...');
