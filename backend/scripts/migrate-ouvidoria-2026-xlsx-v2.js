/**
 * Script de Migração: Base Ouvidoria 2026 (XLSX) → MongoDB reclamacoes_n2Pix
 ...
 * - Coluna B: Finalizado.dataResolucao (updatedAt) - se houver valor, Finalizado.Resolvido = true
 ...
 * 
 * Uso:
 *   node backend/scripts/migrate-ouvidoria-2026-xlsx-v2.js [--dry-run]
 */
 ...
 */
async function main() {
  console.log('🚀 Iniciando migração Base Ouvidoria 2026 (XLSX) → MongoDB...');
  ...
  try {
    ...
    // Verificar se collection já tem dados (de Ouvidoria 2025)
    if (!DRY_RUN) {
      const count = await collection.countDocuments();
      if (count > 0) {
        console.log(`⚠️  Collection já contém ${count} documentos. Não será limpa (Ouvidoria 2025 já inseriu dados).\n`);
      } else {
    ...
    if (documentos.length > 0) {
      if (!DRY_RUN) {
        ...
        // Criar índices (apenas se collection estava vazia)
        const countBefore = await collection.countDocuments();
        if (countBefore === documentos.length) {
